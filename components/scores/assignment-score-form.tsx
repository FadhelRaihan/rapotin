...
interface AssignmentScoreFormProps {
  students: Student[];
  subjects: Subject[];
  initialScores: AssignmentScore[];
}

export function AssignmentScoreForm({
  students,
  subjects,
  initialScores,
}: AssignmentScoreFormProps) {
  const router = useRouter();
  const [activeTab, setActiveTab] = useState<'batch' | 'history' | 'pending'>('batch');
  const [scores, setScores] = useState<AssignmentScore[]>(initialScores);
  ...
  // Keyboard Navigation in Batch Mode
  ...
  const handleBatchSubmit = async (e: React.FormEvent) => {
    ...
    if (lErr) {
      setLabelError(lErr);
      setError(`Kesalahan pada Nama / Label: ${lErr}`);
      return;
    }
    ...
    // Build scores list for ALL students in classroom
    ...
    try {
      const res = await fetch('/api/scores/assignment/batch', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...batchHeader,
      ...
      // Set History filter to match the submitted assignment
      setHistoryFilter({
      ...
      setBatchHeader((prev) => ({ ...prev, label: '' }));
  ...
  const handleSavePendingScore = async (scoreId: string) => {
    ...
    try {
      const res = await fetch(`/api/scores/assignment/${scoreId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Gagal menyimpan nilai susulan');
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Apakah Anda yakin ingin menghapus catatan nilai tugas ini?')) return;
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <div className="flex items-center gap-1.5 text-xs text-[#6B7280] mb-1">
            ...
            <span className="text-[#111827] font-medium">Nilai Tugas</span>
          </div>
          <h1 className="text-2xl md:text-3xl font-bold text-[#111827] tracking-tight">
            Input Nilai Tugas Siswa
          </h1>
          <p className="text-sm text-[#6B7280] mt-1">
            Mode cepat input nilai tugas 1 kelas sekaligus atau lihat riwayat nilai
          </p>
        </div>
        ...
        <div className="flex items-center p-1 bg-[#F8FAFC] border border-[#E5E7EB] rounded-xl w-fit">
          ...
          >
            ...
            <span>+ Input Nilai Baru (Batch)</span>
          </button>
          ...
          >
            ...
            <span>📋 Riwayat Nilai ({validHistoryScores.length})</span>
          </button>
          ...
          {pendingScores.length > 0 && (
            ...
            >
              ...
              <span>⌛ Nilai Susulan ({pendingScores.length})</span>
            </button>
          )}
        </div>
      </div>
        ...
        <form onSubmit={handleBatchSubmit} className="flex flex-col gap-6">
          ...
          <Card className="p-6 border-[#E5E7EB] bg-white rounded-2xl shadow-xs flex flex-col gap-4">
            <div className="flex items-center gap-2 pb-3 border-b border-[#E5E7EB]">
              ...
              <div>
                <h2 className="text-base font-bold text-[#111827]">
                  1. Informasi Tugas Yang Akan Dinilai
                </h2>
                <p className="text-xs text-[#6B7280]">
                  Pilih mata pelajaran dan tentukan label tugas 1x untuk seluruh kelas
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <Input
                label="Nama / Label Tugas *"
                placeholder="Misal: Tugas 1 - Pecahan"
          ...
          <Card className="p-0 overflow-hidden border border-[#E5E7EB] rounded-2xl bg-white shadow-xs flex flex-col">
            <div className="p-4 bg-[#F8FAFC] border-b border-[#E5E7EB] flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                <h3 className="text-sm font-bold text-[#111827]">
                  2. Lembar Input Nilai Kelas ({students.length} Siswa)
                </h3>
                <p className="text-xs text-[#6B7280]">
            ...
            <div className="p-4 bg-[#F8FAFC] border-t border-[#E5E7EB] flex justify-end">
              <Button type="submit" disabled={loading || filledCount === 0 || hasScoreErrors || Boolean(labelError)} size="md">
                ...
                <span>{loading ? 'Menyimpan Semua...' : `Simpan ${filledCount} Nilai Tugas (1 Klik)`}</span>
              </Button>
            </div>
          </Card>
        </form>
        ...
        <div className="flex flex-col gap-4">
          ...
          <Card className="p-4 border-[#E5E7EB] bg-white rounded-2xl shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-2 text-xs font-bold text-[#111827]">
              ...
              <span>Filter Riwayat Nilai:</span>
            </div>
          ...
          <Card className="p-0 overflow-hidden border border-[#E5E7EB] rounded-2xl bg-white shadow-xs">
            <div className="p-4 bg-[#F8FAFC] border-b border-[#E5E7EB] flex items-center justify-between">
              <div className="text-xs font-bold text-[#111827]">
                Menampilkan {filteredHistoryScores.length} Data Nilai Tugas Terisi
              </div>
              ...
              <table className="w-full text-left border-collapse text-sm min-w-[700px]">
                <thead>
                  <tr className="border-b border-[#E5E7EB] bg-[#F8FAFC] text-xs font-semibold text-[#6B7280]">
                    ...
                    <th className="p-4">Judul / Label Tugas</th>
                  ...
                  {filteredHistoryScores.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="p-8 text-center text-[#6B7280] text-xs">
                        Tidak ada riwayat nilai tugas yang sesuai dengan filter yang dipilih.
                      </td>
                    </tr>
                  ) : (
          ...
          <div className="p-4 bg-[#F8FAFC] border-b border-[#E5E7EB] flex items-center justify-between">
            <div>
              <h3 className="text-sm font-bold text-[#111827] flex items-center gap-2">
                ...
                Daftar Nilai Tugas Belum Terisi ({pendingScores.length} Siswa)
              </h3>
              <p className="text-xs text-[#6B7280]">
                Siswa yang nilainya dikosongkan saat batch input. Masukkan nilai susulan jika siswa sudah menyelesaikan tugas.
              </p>
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse text-sm min-w-[700px]">
              ...
              <tbody className="divide-y divide-[#E5E7EB]">
                {pendingScores.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="p-8 text-center text-[#6B7280] text-xs">
                      🎉 Selamat! Seluruh nilai tugas siswa telah terisi lengkap.
                    </td>
                  </tr>
                ) : (
